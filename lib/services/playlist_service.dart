...
/// Persists user video playlists locally.
class PlaylistService {
  PlaylistService._();

  static const _storageKey = 'video_playlists_v1';
  static final _random = Random();

  static String _newId() =>
      '${DateTime.now().millisecondsSinceEpoch}_${_random.nextInt(1 << 32)}';

  static Future<List<VideoPlaylist>> loadAll() async {
    final prefs = await SharedPreferences.getInstance();
    final raw = prefs.getString(_storageKey);
    if (raw == null || raw.isEmpty) return <VideoPlaylist>[];
    try {
  ...
  static Future<void> _saveAll(List<VideoPlaylist> playlists) async {
    final prefs = await SharedPreferences.getInstance();
    final encoded = jsonEncode(playlists.map((p) => p.toJson()).toList());
    await prefs.setString(_storageKey, encoded);
  }

  static Future<VideoPlaylist> create(String name) async {
    final trimmed = name.trim();
    final playlists = List<VideoPlaylist>.from(await loadAll());
  ...
  static Future<void> reorder(List<VideoPlaylist> ordered) async {
  ...
  static Future<VideoPlaylist?> byId(String id) async {
