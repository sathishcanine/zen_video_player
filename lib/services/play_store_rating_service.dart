...
/// Play Store rating prompts: after rewarded download, and on the home folder list.
class PlayStoreRatingService {
  ...
  /// Schedules the home (folder list) prompt on the next library visit.
  static Future<void> scheduleHomePrompt() async {
    if (!_isEligiblePlatform) return;
    try {
      final prefs = await SharedPreferences.getInstance();
      if (_hasRated(prefs)) return;
